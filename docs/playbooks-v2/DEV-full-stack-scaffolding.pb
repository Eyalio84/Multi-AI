...
**Strategy 9: Story-Driven Interview (18x density) ⭐ NEW**
```
Full 2-section, 20-question structured interview from PB-28:

Section 1: Problem Space (10 questions)
├── Q1-Q5: Open exploratory (what, who, why)
├── Q6: Priority forcing (ONE thing must do well)
├── Q7: Negative space (what causes abandonment)
├── Q8-Q9: Constraints (data, technical)
└── Q10: Success criteria (how to measure)

Section 2: UI/UX Appearance (10 questions)
├── Q1-Q3: Sensory anchoring (mood, references, colors)
├── Q4-Q7: Constrained choices (platform, density, theme)
├── Q8: Primary action identification
└── Q9-Q10: Customization and brand

Output: PRD + Design Brief + Technical Spec
Density: 18x (20 questions → 200+ data points via synthesis)

Best for: Complex apps, unclear requirements, stakeholder alignment
See: [Playbook 28: Story-Driven Architectural Interviews](PLAYBOOK-28-STORY-DRIVEN-ARCHITECTURAL-INTERVIEWS.md)
```

### Adaptive Story Density (3rd Knowledge Insight #768)

The story arc (Problem→Solution→Experience) adapts to communication style:
- **Natural language users**: Full narrative journey, open questions
- **Structured users**: Same arc as constrained multi-select
- **Example-based users**: Arc via reference anchoring

Same story, different compression levels.
...
#          story_driven_interview  # ⭐ NEW (PB-28)
...
elif path == 'story_driven_interview':  # ⭐ NEW
    # Full 20-question story-driven interview (PB-28)
    from story_interview import StoryDrivenInterviewer
    story_interviewer = StoryDrivenInterviewer()

    # Section 1: Problem Space (10 questions)
    problem_answers = story_interviewer.run_section('problem_space')

    # Section 2: UI/UX Appearance (10 questions)
    uiux_answers = story_interviewer.run_section('uiux_appearance')

    # Synthesize into PRD + Design Brief
    prd = story_interviewer.synthesize_prd(problem_answers)
    design_brief = story_interviewer.synthesize_design_brief(uiux_answers)

    # Generate blueprint from synthesized documents
    responses = {'prd': prd, 'design_brief': design_brief}
