...
const StudioChatPanel: React.FC = () => {
  const { messages, streamingText, streamingPlan, streamingThinking, isStreaming, sendMessage } = useStudio();
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {isStreaming && streamingThinking && !streamingText && (
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-lg px-3 py-2 text-sm" style={{ background: 'var(--t-surface)', border: '1px solid var(--t-border)' }}>
              <div className="flex items-center gap-1.5 mb-1">
                <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: 'var(--t-warning)' }}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707" />
                </svg>
                <span className="text-xs font-medium" style={{ color: 'var(--t-warning)' }}>Thinking...</span>
              </div>
              <div className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--t-muted)', maxHeight: '120px', overflow: 'auto' }}>
                {streamingThinking}
              </div>
            </div>
          </div>
